...
var VocabularyParser = function (options, callback) {
	...
	'{' +
      ...
      self.sax_parser.ontext = function (text) {
	    ...
	    if (!self.json.hasOwnProperty(l)) continue;
	...
	// lift the results
	  ...
	  if (!self.json.hasOwnProperty(lang)) continue;
...
VocabularyParser.prototype.parse = function (input_object, callback) {
  ...
  if (!input_object || typeof input_object !== "object") {
    ...
    for (var lang in input_object) {
      if (!input_object.hasOwnProperty(lang)) continue;
	...
	break;
      ...
      for (var p in template) {
        if (!template.hasOwnProperty(p)) continue;
	...
	if (p === "deprecated") {
          if (Array.isArray (input_object[lang][p]) === false) {
...
VocabularyParser.prototype.getVocabulary = function (lang, callback) {
  ...
  var voc = termsapi.createVocabulary (l, self.json[l]); 

  if (callback && typeof callback === "function") {
    return callback (error, voc);
  } else {
    if (error) {
      console.error (error);
      return;
